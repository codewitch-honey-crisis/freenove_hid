...
void led_initialize() {
    if (led_initialized || i2c_bus_handle!=NULL) {
        return;
    }
...
static void i2c_initialize() {
    ...
    memset(&config, 0, sizeof(config));
    config.clk_source=I2S_CLK_SRC_DEFAULT;
...
static int touch_write_reg(int r, int value) {
    ...
    if(ESP_OK==i2c_master_transmit(touch_handle,buf,sizeof(buf),1000)) {
        return 1;
    }
    return 0;
}
static int touch_read_all() {
    ...
    // Read data
    uint8_t tmp = 0;
    i2c_master_transmit(touch_handle,&tmp,1,1000);
    i2c_master_receive(touch_handle,i2cdat,sizeof(i2cdat),1000);
...
void touch_initialize(int threshhold) {
    ...
    dev_config.device_address = 0x38;
    dev_config.scl_speed_hz = 200*1000;
    ESP_ERROR_CHECK(i2c_master_bus_add_device(i2c_bus_handle, &dev_config,&touch_handle));
...
void touch_deinitialize() {
    ...
    ESP_ERROR_CHECK(i2c_master_bus_rm_device(touch_handle));
    touch_handle = NULL;
}
